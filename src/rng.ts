...
function randomRange(min: number, max: number): number {
  if (!Number.isInteger(min) || !Number.isInteger(max)) {
    throw new RangeError(`randomRange: min and max must be integers, got ${min} and ${max}`);
  }
  if (min > max) {
    throw new RangeError(`randomRange: min must be <= max, got ${min} > ${max}`);
  }
  if (min === max) return min;

  const range = max - min + 1;
  if (range > 2 ** 32) {
    throw new RangeError(`randomRange: range (max - min + 1) must be <= 2^32, got ${range}`);
  }
