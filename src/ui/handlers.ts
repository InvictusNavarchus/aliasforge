import { type DOBFormat, DOBGen } from '../dob-gen.ts';
import { type FullName, NameGen } from '../name-gen.ts';
import {
	type UsernameCasing,
	UsernameGen,
	type UsernamePattern,
} from '../username-gen.ts';
import { Renderer } from './renderer.ts';

const globalCountSlider = document.getElementById(
	'global-count',
) as HTMLInputElement;

const usernamePatternSelect = document.getElementById(
	'username-pattern',
) as HTMLSelectElement;
const usernameCaseSelect = document.getElementById(
	'username-case',
) as HTMLSelectElement;
const usernameMatchSelect = document.getElementById(
	'username-match',
) as HTMLSelectElement;
...
const dobFormatSelect = document.getElementById(
	'dob-format',
) as HTMLSelectElement;

const resultsNames = document.getElementById('results-names') as HTMLElement;
const resultsUsername = document.getElementById(
	'results-username',
) as HTMLElement;
...
function generateUsernames(): void {
	...
	if (match) {
		const renderedNames = Array.from(
			resultsNames.querySelectorAll('.result-value'),
		);
