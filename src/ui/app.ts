...
const generateAllStrip = document.getElementById(
	'generate-all-strip',
) as HTMLElement;
...
const globalCountDisplay = document.getElementById(
	'global-count-display',
) as HTMLElement;

const btnNames = document.getElementById('btn-names') as HTMLButtonElement;
const btnUsername = document.getElementById(
	'btn-username',
...
async function init(): Promise<void> {
	try {
		...
		setStatus(
			`Loaded ${firstNames.toLocaleString()} first names · ${lastNames.toLocaleString()} last names`,
		);
		showApp();
